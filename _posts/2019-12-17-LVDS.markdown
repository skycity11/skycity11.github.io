...
LVDS_TX IP核配置（结合实例）
* LVDS_TX的目的是将并行的数据转化成高速串行的串行数据，串行数据的传输速率取决于输入时钟、串化因子和通道数。若为10位并行数据，则将输入时钟10倍频，每个上升沿传一位数据即可。此时串化因子为10，通道数为1。
  
  但串化因子的上限位10，如果并行数据是10位以上，比如为21位，此时只能增加通道数。将输入时钟7倍频，每一位
